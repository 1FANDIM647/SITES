...
public:
	...
	olcSpirite (int w, int h)
	{
		Create (w, h);
	}

	olcSpirite(std::wstring sFile)
	{
		if (!Load(sFile))
			Create(8, 8);
	}
...
private:
     ...
     void Create(int w, int h )
     {
     	...
     	m_Golours = new short[w*h];
     	for (int i = 0; i < w*h; i++)
     	{
     		m_Glyphs[i] = L ' ' ;
     		m_Golours[i] = FG_BLACK;
     	}
     }
public:
    ...
    bool Save(std::wstring sFile)
    {
    	...
    	fwrite(m_Golours, sizeof(short), nWidth*nHeight,f);
    ...
    bool Load(std::wstring sFile )
    {
    	...
    	std::fread(m_Golours , sizeof(short), nWidth * nHeight , f);
